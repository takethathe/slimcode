...
每轮把「system 提示 + 工具定义」以及自尾部扫描到的最后一条非空对话消息打上标记，
让端点缓存稳定的完整历史前缀；后续轮次追加新消息后，此前历史命中、只为新消息计费。
关闭缓存后请求字节与不开启缓存的客户端完全一致。缓存命中的 token 数与命中百分比
（`cached / prompt`）会显示在运行结束的 token 用量汇总行中（`({cached} cached, {pct}%)`）。
