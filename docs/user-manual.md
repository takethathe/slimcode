...
显式上下文缓存默认开启：system 消息（含工具定义）加上自尾部扫描到的最后一条非空对话
消息作为稳定前缀交给端点缓存，多轮复用同一前缀时命中缓存，后续轮次只为新消息计费。
因此「system 前缀」与「完整对话前缀」都进入缓存，长会话的输入成本随轮次摊薄。
...
- 注意：创建缓存本身按 125% 输入价计费一次；多轮命中后由命中价回收成本。
  缓存前缀过短（< 1024 token）时可能不会实际命中，属端点运行行为。
...
- **滚动**：transcript 滚动超出一屏时，右侧出现滚动条拇指（滚动后约 1 秒自动淡出），
  `PgUp`/`PgDn` 翻页，**鼠标滚轮**上/下滚动 transcript（3 行/格：上滚即停止跟随新输出、
  滚到底自动恢复跟随）；上滚后视图会停在原处，新流式输出**不会**把你拽回底部（所以在
  agent 跑一轮的中途也能回看历史）；滚轮是纯粹的「看内容」手势——**永不**进入输入历史 recall、不提交
  prompt、不跑命令、不改会话，指针停在哪里都一样（压在输入框或 footer 上也照样滚）；
  **一轮运行中也能滚**（滚轮不打断在途请求、也不取消这一轮）；
  `/` 补全弹框打开时滚轮仍然只滚 transcript（弹框用 `↑`/`↓`/`PgUp`/`PgDn` 导航）；
...
- **Footer**（dock 底部两行 dim）：第一行 `~/cwd (branch) • session-id`（在 git 仓库内时
  ...
  后跟 context 占用段 `45.3%/200k`（= 估算 token / 200k 窗口，用与自动压缩同一估算器），
  再是右对齐的模型名；超宽时先截模型名、极端情况再连统计+context 一起截。context 段按
  等级着色：≤70% dim、>70% 黄、>90% 红（统计与模型名始终 dim）；每条 assistant 回复后更新，
  启动/`/new` 显示 `0%/200k`，切换会话后由 CLI 重新估算后补发（无上个会话残留）；
...
| 鼠标滚轮 | 上/下滚动 transcript（3 行/格，触发/维持滚动条显示）；指针位置无关，`/` 补全弹框打开时仍只滚 transcript；**永不**进入输入历史 recall |
...
滚动 transcript 需要 TUI 订阅鼠标事件，因此有几个终端侧的行为变化（都不是 bug）：

- **原生选区要按 Shift+拖拽**：订阅鼠标后，普通拖拽交给 TUI，不再触发终端原生选区；
  在 Ghostty 下按住 **Shift** 拖拽仍是原生选区（其 `mouse-shift-capture` 默认 `false`，
  Shift 不下发给 app）。想整体关掉 app 的鼠标捕获，可用 Ghostty 的
  `mouse-reporting = false` 或 `toggle_mouse_reporting` 键位——代价是滚轮也一起失效；
- **tmux 下需 `set -g mouse on`**：否则 tmux 截走滚轮，事件到不了 slimcode；
- **iTerm2 的 fast trackpad** 会丢滚轮增量（手感变差），建议关闭该选项；
- **点击不再起作用**：左键的按下/释放不会移动输入框光标、不展开/折叠工具块、不选择
  补全候选；
- 鼠标移动（motion）与水平滚轮未被订阅，不产生任何效果。
...
- 只要没用 `↑`/`↓`/`PgUp`/`PgDn` 手动移动过，预选高亮会随每次输入**自动跟随当前最匹配项**
  （即使上一轮的高亮项仍在候选里）；手动移动后，高亮粘住你选中的值，直到它被过滤掉或弹框
  关闭；
- 输入 `/` 后无候选时弹框不显示。

弹框只在前端 TUI 出现；候选池由 CLI 在启动时注入（`slimcode-commands` 的命令表 +
skills store），模糊匹配与候选合并仍是前端无关的 `slimcode-commands` /
`slimcode-app` 里的纯函数（见 ADR-0005、ADR-0014 D3），可独立单测。

**命令**（唯一命令集，由 CLI 解析并用注册表 `slimcode-commands` 校验拼写）：
...
「这个会话这次被用掉多少」。但 footer 的 **context 占用段**例外：它由消息历史估算，
不依赖不落盘的累计，所以载入会话后按恢复的历史重新估算（并补发一次），同一会话内
`/new` 则回到 `0%/200k`。
...
token 数（`字符数 / 4` 的保守启发式，连同工具调用的参数一起算）；一旦只剩不到 **16k**
token 的剩余上下文（即估算总量 ≥ 200k 窗口 − 16k），就在后台自动压缩：

- 调用同一个 provider 生成一段**结构化摘要**（`## Goal` / `## Constraints & Preferences` /
  `## Progress` / `## Key Decisions` / `## Next Steps` / `## Critical Context`），把较旧的消息
  替换为一条会话内部的 **CompactSummary**，并保留最近约 16k token（窗口 8%）的历史原样（该尾部只在本进程内
...
不想等阈值也可以手动触发 **`/compact`**：它跳过阈值判断，立即压缩并可读出
...
如果弹框未打开而仍提交了未知的 `/` 命令，TUI 会显示**预测提示**（CLI 用前端无关的命令
注册表 `slimcode-commands` + 已安装 skill 生成，经 notice/error 行到达）：
...
未来其它前端（Web / RPC 等）可复用同一套命令定义、补全与上下文组装逻辑。**Skill** 也以 `/` 触发，
...
记录或修补的工具调用，会给出相应 notice。**加载后**，该会话的历史消息会重放到 transcript
（用户 prompt 框、assistant 文本、成对的工具调用/结果块），屏幕重新显示这段对话而不是
停在 header；之后继续的新消息按常规追加。继续对话时，历史消息（含系统提示与
`## Environment` 环境信息）原样作为上下文传入，环境沿用首轮冻结值。
...
`find`、`ls`。一次响应内的多个工具调用默认**并行**执行（各自独立线程，事件按完成
顺序显示、结果按模型返回顺序写入会话历史）；模型也可对相互依赖的任务返回单个调用、
多轮串行完成。工具失败会以 `Error: …` 反馈给模型供其自行纠正。
