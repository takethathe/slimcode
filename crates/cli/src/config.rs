//! Application configuration: defaults < `config.toml` < environment.
//!
...
//! env (`SLIMCODE_AI_BASE_URL` / `SLIMCODE_AI_MODEL`) > config.toml > defaults
//! from `slimcode_ai`.
...
/// Resolve one config value: env > file > default.
fn pick_value(file: Option<&str>, env: Option<String>, default: &str) -> String {
    env.or_else(|| file.map(str::to_string))
        .unwrap_or_else(|| default.to_string())
}

/// Pure resolution core: given optional file contents and an env lookup,
/// return the resolved config. Separated from I/O for unit testing.
fn resolve(
    ...
    let base_url = pick_value(file_base_url, env(ENV_BASE_URL), DEFAULT_BASE_URL);
    let model = pick_value(file_model, env(ENV_MODEL), DEFAULT_MODEL);
...
impl AppConfig {
    /// Load config from the real environment and `$SLIMCODE_HOME/config.toml`
    /// (or `~/.slimcode/config.toml`). The file is optional.
    pub fn load() -> Result<Self, String> {
        let path = slimcode_home().map(|h| h.join("config.toml"));
        Self::load_from(path.as_deref())
    }

    /// Load config with an explicit config file path. `None` (or a missing
    /// file) skips file loading entirely.
    pub fn load_from(path: Option<&Path>) -> Result<Self, String> {
        ...
        resolve(file_toml.as_deref(), &|k| std::env::var(k).ok())
    }
}
...
mod tests {
    ...
    fn file_overrides_defaults() {
        ...
        let cfg = resolve(Some(file), &env).unwrap();
